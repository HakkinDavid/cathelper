...
<div class="flex flex-col justify-items-end px-2 pt-2 w-full h-full">
    <div class="flex flex-row justify-between items-center place-content-center flex-wrap">
        <div class="w-1/3 flex flex-col">
            ...
            <div class={input.div}>
                ...
                <input class={input.self} type="number" bind:value={problem.cable.ctension} min=0>N
    ...
    <div class="mt-2 rounded-lg bg-amber-900 p-2">
        {#if (problem.cable.ctension == 0 || problem.cable.tension.toString() == 'NaN' || problem.cable.tension.toString() == "Infinity")}
            <p class="font-['Oswald'] text-center text-xl text-white">Ingrese más datos.</p>
        ...
        {:else if (problem.cable.ctension - problem.cable.tension) >= 0}
            <p class="font-['Oswald'] text-center text-xl text-yellow-500">No debería operar la grúa en condiciones reales, ya que cualquier fuerza incidente en el eje Y hacia abajo puede volcar su equipo o romper el cable. Se recomienda cambiar el cable por uno de {(problem.cable.tension+(problem.cable.tension*0.10)).toFixed(4)}N.</p>
        {:else if (problem.cable.ctension - problem.cable.tension) < 0}
            <p class="font-['Oswald'] text-center text-xl text-red-500">¡ALTO! No intente operar la grúa. Su cable es demasiado débil. Se recomienda cambiar el cable por uno de {(problem.cable.tension+(problem.cable.tension*0.10)).toFixed(4)}N.</p>
        {/if}        
    </div>
    <br>
</div>